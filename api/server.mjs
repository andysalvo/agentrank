...
createServer((req, res) => {
  ...
  if (p === '/' || p === '/v1') {
    return json(res, 200, {
      ...
      endpoints: { index: '/index?limit=50&role=service', top: '/top?limit=20', rank: '/v1/rank/{address}', score: '/score/{address}', methodology: REPO + '/blob/main/METHODOLOGY.md' },
      publisher: DB.publisher || 'Crest Deployment Systems LLC', repo: REPO,
    });
  }
  if (p === '/health') return json(res, 200, { ok: true, agents: DB.agents.length, generated_at: DB.generated_at });
